...
from models.model_explainer import ModelExplainer, FeatureAnalyzer
...
class CopperPredictionSystem:
    ...
    def __init__(self, data_source: str = "auto"):
        ...
        if data_source == "auto":
            ...
            if self.data_manager.ak.available or self.data_manager.yahoo.available:
                self.data_source_type = "real"
                print("✓ 使用真实数据源")
                if self.data_manager.ak.available:
                    print("  - AKShare可用")
                if self.data_manager.yahoo.available:
    ...
    def prepare_features(self, target_horizon: int = 5) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        target = (close.shift(-target_horizon) / close - 1).loc[features.index]
